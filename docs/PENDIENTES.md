...
| P2-mail | **Correo saliente (más adelante, decisión del dueño)** | (a) Cuenta Resend de NectaCore → `ABI_RESEND_API_KEY`+`ABI_EMAIL_FROM`; los DNS del dominio los agrego por API en cuanto exista. Nota: la cuenta Resend se crea con CUALQUIER correo tuyo (no hace falta buzón @nectacore.com — la verificación del dominio es por DNS, y el remitente `hola@nectacore.com` no necesita buzón real). Si quieres RECIBIR en @nectacore.com sin pagar nada: Cloudflare Email Routing reenvía a tu inbox personal — lo configuro por API cuando digas. (b) Google OAuth: sumar nectacore.com a `GOTRUE_URI_ALLOW_LIST` + redirect en Google Console. Hasta entonces el formulario de claim responde con gracia (503 amable). | Más adelante |
| P-WA | Onboarding WhatsApp self-serve | Diseño completo en [`WHATSAPP-ONBOARDING.md`](WHATSAPP-ONBOARDING.md): del proceso manual de 3 toques (cuenta + código + QR por chat) a 0 toques vía portal — `POST /v1/profiles` + Embedded Signup por redirect (QR y código viven en el flujo oficial de Meta) + webhooks de confirmación. Implementación cuando exista F1/P3 (es un paso post-upgrade del panel). | Con P3 |
